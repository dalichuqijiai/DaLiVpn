...
MY_IP4=$(curl -s4 --connect-timeout 5 http://ifconfig.me 2>/dev/null \
      || curl -s4 --connect-timeout 5 http://ip.sb 2>/dev/null \
      || curl -s4 --connect-timeout 5 http://icanhazip.com 2>/dev/null)

if [ -n "$MY_IP4" ]; then
    echo "IPv4 地址: $MY_IP4"
else
    echo "⚠️  未检测到 IPv4 公网地址（可能无 IPv4 或无法访问检测源）"
fi

# IPv4 中国目标（尽量用 HTTP 避免证书干扰）
TARGETS_V4=("http://baidu.com" "http://www.qq.com")
...
for site in "${TARGETS_V4[@]}"; do
    HTTP_CODE=$(curl -o /dev/null -s -L --connect-timeout 5 -w "%{http_code}" "$site" 2>/dev/null || echo "000")
    ((V4_TOTAL++))
...
if [ "$IPV6_AVAILABLE" = true ]; then
    MY_IP6=$(curl -s6 --connect-timeout 5 http://ifconfig.me 2>/dev/null \
          || curl -s6 --connect-timeout 5 http://ip.sb 2>/dev/null \
          || curl -s6 --connect-timeout 5 http://icanhazip.com 2>/dev/null)
fi

if [ -n "$MY_IP6" ]; then
    echo "IPv6 地址: $MY_IP6"
else
    echo "⚠️  未检测到公网 IPv6 地址（VPS 可能没有 IPv6，或检测源不可达）"
fi
...
if [ -n "$MY_IP6" ]; then
    for site in "${TARGETS_V6[@]}"; do
        HTTP_CODE=$(curl -o /dev/null -s -6 -L --connect-timeout 5 -w "%{http_code}" "$site" 2>/dev/null || echo "000")
        ((V6_TOTAL++))
        if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "301" ] || [ "$HTTP_CODE" = "302" ]; then
