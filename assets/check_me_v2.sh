...
if [ -n "$MY_IP6" ]; then
    echo "  ⏳ 正在测试 IPv6 连通性（可能需要稍长时间）..."

    for site in "${TARGETS_V6[@]}"; do
        ((V6_TOTAL++))

        # 方法1: curl IPv6 HTTP（超时较长）
        HTTP_CODE=$(curl -o /dev/null -s -6 --connect-timeout 8 --max-time 15 -w "%{http_code}" "$site" 2>/dev/null)
        ...
        # 方法2: 兜底 HTTPS（有些站点 IPv6 HTTP 响应慢，HTTPS 更稳定）
        site_https="${site/http:/https:}"
        HTTP_CODE=$(curl -o /dev/null -s -6 -k --connect-timeout 8 --max-time 15 -w "%{http_code}" "$site_https" 2>/dev/null)
        if [ -n "$HTTP_CODE" ] && { [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "301" ] || [ "$HTTP_CODE" = "302" ]; }; then
            echo "  ✅  [IPv6 SUCCESS] $site_https  (HTTP $HTTP_CODE)"
            ((V6_OK++))
            continue
        fi

        # 方法3: ping6 兜底，验证基本路由可达
        if command -v ping6 &>/dev/null || ping -6 -c 1 ::1 &>/dev/null 2>&1; then
            PING_CMD="ping -6 -c 3 -W 5"
        elif command -v ping6 &>/dev/null; then
            PING_CMD="ping6 -c 3 -W 5"
        else
            PING_CMD=""
        fi
        ...
        if [ -n "$PING_CMD" ]; then
            ...
            if $PING_CMD "$domain" &>/dev/null; then
                echo "  ✅  [IPv6 SUCCESS] ping6 $domain  (路由可达)"
                ((V6_OK++))
                continue
            fi
        fi

        echo "  ❌  [IPv6 FAILED]  $site  (所有检测方式均失败)"
    done
else
